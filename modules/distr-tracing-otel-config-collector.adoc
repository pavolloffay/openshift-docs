    ...
    service:
      ...
      pipelines:
        traces:
          ...
          exporters: [otlp]
----
<1> The client identifier for the OpenTelemetry Collector application.
<2> The secret that is shared between the client and authorization server.
<3> You can add further metadata as key-value pairs to be transferred during authentication.
<4> The token endpoint that is used to retrieve an access token.
